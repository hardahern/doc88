...
class converter:
    ...
    def set_swf(self, i: int, w, h):
        return subprocess.run(
                ["java", "-Xmx512m", "-Xms64m", "-jar", "ffdec/ffdec.jar", "-header", "-set", "width", f"{w}px", "-set", "height", f"{h}px", f"{cfg2.swf_path}{i}.swf", f"{cfg2.swf_path}{i}.swf"],
                capture_output=True,
                text=True,
                timeout=60,
            )
        ...
        try:
            ...
            for f in os.listdir(ospath(dirpath)):
                if os.path.isdir(ospath(f"{dirpath}{f}")):
                    src = ospath(f"{dirpath}{f}/1.svg")
                    if not os.path.isfile(src) or os.path.getsize(src) == 0:
                        print(f"页 {f[:-4]} 生成 SVG 为空，跳过。")
                        logw(f"Empty SVG for {f}: ffdec stdout tail={log[-500:]}")
                        continue
                    shutil.move(
                        src, ospath(f"{cfg2.svg_path}{f[:-4]}.svg")
                    )
            # 删除ffdec的临时文件夹
            try:
    ...
    def swf2pdf(self, i: int):
        ...
        try:
            ...
            for f in os.listdir(ospath(dirpath)):
                if os.path.isdir(ospath(f"{dirpath}{f}")):
                    src = ospath(f"{dirpath}{f}/frames.pdf")
                    if not os.path.isfile(src) or os.path.getsize(src) == 0:
                        print(f"页 {f[:-4]} 生成 PDF 为空，跳过。")
                        logw(f"Empty PDF for {f}: ffdec stdout tail={log[-500:]}")
                        continue
                    shutil.move(
                        src, ospath(f"{cfg2.pdf_path}{f[:-4]}.pdf")
                    )
    ...
    def svg2pdf(self, i: int):
        try:
            ...
            # )
            run=subprocess.run(
                ["./svg2pdf", f"{cfg2.svg_path}{i}.svg", f"{cfg2.pdf_path}{i}.pdf"], text=True, capture_output=True, timeout=120
            )
            ...
            print(f"svg2pdf for page {i} 超时已跳过。")
            logw(f"SVG to PDF timeout for page {i}")
    ...
    def makepdf(self):
        ...
        for i in self.pdflist:
            path = ospath(f"{cfg2.pdf_path}{i}.pdf")
            if not os.path.isfile(path) or os.path.getsize(path) == 0:
                skipped.append(i)
                continue
            self.pdf = append_pdf(self.pdf, str(path))
            appended += 1
        if skipped:
            print(f"警告: 跳过空 PDF 页 {skipped}")
            logw(f"makepdf skipped empty pages: {skipped}")
        if appended == 0:
            raise Exception("没有任何页 PDF 转换成功，缓存将保留以便排查。")
