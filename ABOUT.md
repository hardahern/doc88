...
很明显，要先选择PK文件，对吗？提示检测到`YBD`头部，看起来很对，再选择PH文件，也检测到了，再让我们保存文件，就这样成功了？不对，转出来的文件没有任何内容：
...
所以干掉了 `cairosvg` 转换方式，全面使用 `ffdec`<del>（其实 cairosvg 方案还保留着，只是作为一个备选方案在配置文件里）</del>更新：换为 Rust 编写的 `swf2svg` 工具实现 SVG -> PDF 转换
